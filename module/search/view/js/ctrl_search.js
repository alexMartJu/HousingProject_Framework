...
function load_category(h_type) {
    ...
    if (h_type == undefined) {
        ajaxPromise(friendlyURL('?module=search'), 'POST', 'JSON', {'op':'search_category_null'})
            .then(function (data) {
        ...
        h_type.op = 'search_category';
        ajaxPromise(friendlyURL('?module=search'), 'POST', 'JSON', h_type)
            .then(function (data) {
                for (row in data) {
                    $('<option value="' + data[row].name_category + '">' + data[row].name_category + '</option>').appendTo('.search_category')
                }
...
function autocomplete() {
    ...
    $("#autocom").on("keyup", function () {
        ...
        sdata.op = 'autocomplete';
        ajaxPromise(friendlyURL('?module=search'), 'POST', 'JSON', sdata)
            .then(function (data) {
                // $('#searchAuto').empty();
